...
void loop() {
  while (NodeSerial.available() > 0) 
  {
   ...
   if(forceOnWater <= 0 && v6_autoWater == 1)
   {
      if(M <= v30_turnOnWaterPoint && v7_waterIndicator == 0)
      {
        ...
        Line_Notify("เปิดน้ำ (อัตโนมัติ) 2");
      }
      else if(M >= v31_turnOffWaterPoint && v7_waterIndicator == 1)
      {
         ...
         Serial.println(M);
           ...
           Line_Notify("ปิดน้ำ (อัตโนมัติ) 2");
      }
   ...
   if(T < v32_tempNotifyLow  || T > v33_tempNotifyHigh){  
    if(!tempNotify){
      tempNotify = true;
      Line_Notify("อุณภูมิ 2: " + String(T) +" °C");
    }      
   }
   else
  {
  ...
  if(H < v36_humNotifyLow || H > v37_humNotifyHigh){  
    if(!humNotify){
      humNotify = true;
      Line_Notify("ความชื้นสัมพัทธ์ 2: " + String(H) + "%");
    }      
   }
   else
  {
  ...
  if(CO2 < v38_co2NotifyLow || CO2 > v39_co2NotifyHigh){  
    if(!co2Notify){
      co2Notify = true;
      Line_Notify("CO2 2: " + String(CO2) + " ppm");
    }      
   }
   else
  {
...
void myTimerEvent()
{
  ...
  Blynk.virtualWrite(V12, PH);
  Blynk.virtualWrite(V13, M);
  Blynk.virtualWrite(V14, CO2);
  Blynk.virtualWrite(V15, O2);
